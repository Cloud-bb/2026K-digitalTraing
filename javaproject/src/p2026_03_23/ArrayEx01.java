package p2026_03_23;

public class ArrayEx01 {

	public static void main(String[] args) {
//		배열: 동일한 자료형의 데이터를 저장하기 위한 정적인 자료구조
//		변수  --> 배열  --> List(자료구조)
		
//		1차원 배열 첫 번째 형식
//		:배열에 저장될 값이 정해져 있지 않은 경우에 사용하는 방식
		
		int score[] = new int[3]  ;
//		자료형   배열변수 = 연산자  자료형[배열의 크기];
//		int 배열은 자동으로 0으로 초기화 된다.
		                                      
		System.out.println(score[0]);
		System.out.println(score[1]);
		System.out.println(score[2]);
		
//		배열에 새로운 값을 할당
		score[0]=80;
		score[1]=90;
		score[2]=100;

		System.out.println(score[0]);
		System.out.println(score[1]);
		System.out.println(score[2]);
		
//		double형 배열은 자동으로 0.0으로 초기화된다.
		double[] d= new double[3];
		System.out.println(d[0]);
		System.out.println(d[1]);
		System.out.println(d[2]);
		
//		boolean 배열은 자동으로 false 값으로 초기화된다.
		boolean[] b = new boolean[3];
		System.out.println(b[0]);
		System.out.println(b[1]);
		System.out.println(b[2]);
		
//		String 배열은 자동으로 null로 초기화된다
		String[] str = new String[3];
		System.out.println(str[0]);
		System.out.println(str[1]);
		System.out.println(str[2]);
		
		str[0]="자바";
		str[1]="오라클";
		str[2]="스프링";
		System.out.println(str[0]);
		System.out.println(str[1]);
		System.out.println(str[2]);
		
//		2. 1차원 배열 두번째 형식
//		:배열 선언과 동시에 초기화를 수행할 때 사용하는 형식
//		배열에 할당될 값이 정해져 있는 경우에 주로 사용하는 형식
		
		int[] s1 = {80, 90, 100};
		int[] s2 = new int[] {80, 90, 100};
		System.out.println(s1[0]);
		System.out.println(s1[1]);
		System.out.println(s1[2]);
		
		for(int i=0; i<s1.length; i++) {
			System.out.print(s1[i]+"\t");
		}
		
		double[] dd = {3.14, 10.5, 42.195, 50};
		for(int i =0; i<dd.length;i++) {
			System.out.print(dd[i]+"\t");
		}
		System.out.println();
		
		char[] cc= {'j','a','v','a','자','바'};
		for(int i=0;i<cc.length; i++) {
			System.out.print(cc[i]+"\t");
		}
		System.out.println();
		
		boolean[] bb= {true,false,true};
		for(int i =0;i<bb.length;i++) {
			System.out.print(bb[i]+"\t");
		}
		System.out.println();
		
		String[] str1 = {"자바","오라클","스프링","파이썬","텐스플로우"};
		String[] str2 = new String[] {"자바","오라클","스프링","파이썬","텐스플로우"};
		for(int i =0;i<str1.length;i++) {
			System.out.print(str1[i]+"\t");
		}
		
	}

}
